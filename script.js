...
function updateDeleteButtons() {
  ...
  deleteActiveBtn.disabled = list.children.length === 0;
  deleteCompletedBtn.disabled = completedList.children.length === 0;
}

// Delete All Active Tasks
document
  .getElementById("deleteActiveBtn")
  .addEventListener("click", function () {
    const confirmDelete = confirm("Delete all active tasks?");
    if (confirmDelete) {
      list.innerHTML = ""; // Clear active list
      saveTasks();
      updateDeleteButtons();
    }
  });
